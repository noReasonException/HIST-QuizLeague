...
<div class="Context">
    ...
    <div class="MainMenu" style="background: none;">
        <div class="QuestionGetInfo">
            Επιλεξτε παράγραφο ...<br>

            <?php
            global $connectionObject;
            if(isset($_POST['chapter_id']) and is_numeric((int)$_POST['chapter_id'])){
                $ShowChoicesStatus=ShowChoices('ConfirmButton','sub_chapter_id','sub_chapter_name','select sub_chapter_name,sub_chapter_id from sub_chapters where chapter_id='.$_POST['chapter_id'].';');
                if($ShowChoicesStatus==1){
                    $_SESSION['chapter_id'] = $_POST['chapter_id'];
                    EchoConfirmButton('setquestion.php');
                    AddCategoryButton('schooltype_id','Προσθήκη Παραγράφου');
                }
                else{
