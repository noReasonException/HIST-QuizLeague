...
<div class="Context">
    ...
    <div class="MainMenu" style="background: none;">
        <div class="QuestionGetInfo">
            Επιλεξτε Κεφάλαιο ...<br>

            <?php
            global $connectionObject;
            if(isset($_POST['lesson_id']) and is_numeric((int)$_POST['lesson_id'])){
                if(ShowChoices('ConfirmButton','chapter_id','chapter_name','select chapter_name,chapter_id from chapters where lesson_id='.$_POST['lesson_id'].';')==1){
                    $_SESSION['lesson_id'] = $_POST['lesson_id'];
                    EchoConfirmButton('setsubchapter.php');
                    AddCategoryButton('schooltype_id','Προσθήκη Κεφαλαίου');
                }
                else{
